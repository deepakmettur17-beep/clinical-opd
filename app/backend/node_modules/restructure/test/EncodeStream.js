...
describe('EncodeStream', function() {
  ...
  describe('writeString', function() {
    ...
    it('should encode utf8', function() {
      const expected = Buffer.from('unicode! ðŸ‘', 'utf8');
      const stream = new EncodeStream(new Uint8Array(expected.length));
      stream.writeString('unicode! ðŸ‘', 'utf8');
      assert.deepEqual(stream.buffer, expected);
    });

    it('should encode utf16le', function() {
      const expected = Buffer.from('unicode! ðŸ‘', 'utf16le');
      const stream = new EncodeStream(new Uint8Array(expected.length));
      stream.writeString('unicode! ðŸ‘', 'utf16le');
      assert.deepEqual(stream.buffer, expected);
    });

    it('should encode ucs2', function() {
      const expected = Buffer.from('unicode! ðŸ‘', 'ucs2');
      const stream = new EncodeStream(new Uint8Array(expected.length));
      stream.writeString('unicode! ðŸ‘', 'ucs2');
      assert.deepEqual(stream.buffer, expected);
    });
    ...
    it('should encode utf16be', function() {
      ...
      const stream = new EncodeStream(new Uint8Array(expected.length));
      stream.writeString('unicode! ðŸ‘', 'utf16be');
      assert.deepEqual(stream.buffer, expected);
    });
  });
});
