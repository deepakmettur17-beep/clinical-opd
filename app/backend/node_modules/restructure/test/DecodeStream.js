...
describe('DecodeStream', function() {
  ...
  describe('readString', function() {
    ...
    it('should decode utf8', function() {
      const buf = Buffer.from('unicode! ðŸ‘', 'utf8');
      const stream = new DecodeStream(buf);
      assert.equal(stream.readString(buf.length, 'utf8'), 'unicode! ðŸ‘');
    });

    it('should decode utf16le', function() {
      const buf = Buffer.from('unicode! ðŸ‘', 'utf16le');
      const stream = new DecodeStream(buf);
      assert.equal(stream.readString(buf.length, 'utf16le'), 'unicode! ðŸ‘');
    });

    it('should decode ucs2', function() {
      const buf = Buffer.from('unicode! ðŸ‘', 'ucs2');
      const stream = new DecodeStream(buf);
      assert.equal(stream.readString(buf.length, 'ucs2'), 'unicode! ðŸ‘');
    });
    ...
    it('should decode utf16be', function() {
      ...
      const stream = new DecodeStream(buf);
      assert.equal(stream.readString(buf.length, 'utf16be'), 'unicode! ðŸ‘');
    });
    ...
    it('should decode macroman', function() {
      ...
      const stream = new DecodeStream(buf);
      assert.equal(stream.readString(buf.length, 'mac'), 'Ã¤ccented chÃ¡racters');
    });
