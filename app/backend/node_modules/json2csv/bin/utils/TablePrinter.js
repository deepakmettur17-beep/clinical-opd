...
class TablePrinter {
  ...
  setColumnWidths(line) {
    ...
    this.topLine = `â”Œ${this.colWidths.map(i => 'â”€'.repeat(i)).join('â”¬')}â”`;
    this.middleLine = `â”œ${this.colWidths.map(i => 'â”€'.repeat(i)).join('â”¼')}â”¤`;
    this.bottomLine = `â””${this.colWidths.map(i => 'â”€'.repeat(i)).join('â”´')}â”˜`;
  }
  ...
  formatRow(row) {
    ...
    return Array(height).fill('')
      .map((_, i) => `â”‚${processedCells.map(cell => cell[i]).join('â”‚')}â”‚`)
      .join('\n');
  }
